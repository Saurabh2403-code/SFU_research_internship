...
flags.DEFINE_integer('num_of_channels',128,help='Number Of Base Colour Channels')
...
state_dict=torch.load(model_dir+f'{FLAGS.model_cifar10_weights_step_+{FLAGS.step}.pt}', weights_only=True)
net_model.load(state_dict)
net_model.eval()
with torch.no_grad():
    ...
    with open(save_dir+f'{FLAGS.model}_Distances_{FLAGS.num_images}','a') as f:
        f.write(f'{l2_distance.item()}')
    plt.imshow(l2_distance.clone().numpy)

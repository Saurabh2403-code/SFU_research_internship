...
##DATASET##
flags.DEFINE_string('dataset_address','/home/saurabhg/scratch/LATENT_DATASET/img',help='dataset_address')

flags.DEFINE_string('model','icfm',help='model type')
flags.DEFINE_string('output_dir','/scratch/saurabhg/LATENT_DATASET/flow_outputs/',help='Ouput directory Address')
...
flags.DEFINE_integer('batch_size',99,help='batch_size')
...
def train(argv):
    ...
    ##when data is loaded it is already normalized by the FlatImagedataset
    with torch.no_grad():
        encoded_dataset=vae.encode(dataset).latent_dist
        latents=encoded_dataset.sample()
        latents=(latents*vae.config.scaling_factor).detach()

    latent_dataset=torch.utils.data.TensorDataset(latents)
    latent_dataloader=torch.utils.data.DataLoader(latent_dataset,batch_size=FLAGS.batch_size,shuffle=False)

...
FLAGS = flags.FLAGS
flags.DEFINE_integer('iteration',1,help='iteration')
...
flags.DEFINE_string('output_dir', '/scratch/saurabhg/LATENT_DATASET/flow_outputs_2/', help='Output Directory Address')
flags.DEFINE_string('model', 'icfm', help='model_type')
flags.DEFINE_integer('step', 95000, help='Epoch number after which we are evaluating the model')
...
def inference(argv):
    with torch.no_grad():
        ...
        torch.save(generated_samples,f'{FLAGS.output_dir}/icfm/image_tensor/generated_images_{FLAGS.num_images}_iteration{FLAGS.iteration}_python_inference_scipt.pt')
        torchvision.utils.save_image(generated_samples,f'{FLAGS.output_dir}/icfm/generated_images/generated_images_{FLAGS.num_images}_{FLAGS.step}_python_inference_script_iteration{FLAGS.iteration}.png')

...
FLAGS = flags.FLAGS
flags.DEFINE_integer('iteration','2',help='iteration for the generating images')
...
flags.DEFINE_string('output_dir', '/scratch/saurabhg/CORGIS_DATASET/flow_outputs/', help='Output Directory Address')
flags.DEFINE_string('model', 'icfm', help='model_type')
flags.DEFINE_integer('step', 100000, help='Epoch number after which we are evaluating the model')
...
def inference(argv):
    with torch.no_grad():
        ...
        for i in range(FLAGS.num_images//FLAGS.batch_size):
            ...
            decoded_batch=vae.decode(scaled_latent)
            decoded_batch=decoded_batch.sample
            os.makedirs(f'/home/saurabhg/scratch/CORGIS_DATASET/flow_outputs/icfm/image_tensor/generated_images_{FLAGS.num_images}_iteration{FLAGS.iteration}_python_inference_scipt',exist_ok=True)
            # all_generated.append(decoded_image.cpu())
            torch.save((decoded_batch*0.5+0.5).clip(0,1),f'/home/saurabhg/scratch/CORGIS_DATASET/flow_outputs/icfm/image_tensor/generated_images_{FLAGS.num_images}_iteration{FLAGS.iteration}_python_inference_scipt/part{i}.pt')
            del samples, scaled_latent, decoded_batch
        ...
        # torch.save(generated_samples,f'/home/saurabhg/scratch/CORGIS_DATASET/flow_outputs/icfm/image_tensor/generated_images_{FLAGS.num_images}_iteration{FLAGS.iteration}_python_inference_scipt.pt')
        # torchvision.utils.save_image(generated_samples,f'/home/saurabhg/scratch/CORGIS_DATASET/flow_outputs/icfm/generated_images/generated_images_{FLAGS.num_images}_{FLAGS.step}_python_inference_script_iteration{FLAGS.iteration}.png')
